...
- [x] 6.4 Feedback round: pin the playing station as a list item with its star (card only in the library view), clear the visualizer frame on stop/radio takeover, stop library playback on station activation, fix the `[hidden]` icon fallback override. Verify: `npm run test:e2e` green; manual pass on the real catalog.
