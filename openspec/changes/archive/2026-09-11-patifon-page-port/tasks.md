...
- [x] 1.1 Merge `.progress` + `.bar` into `.deck` (`.deck__strip` with the
...
- [x] 2.1 Rewrite `strip.ts` as one continuous gapless mirrored silhouette
...
- [x] 3.1 Light deck chrome (card fill + metal wash at 40% multiply, deck
...
- [x] 4.1 Rewrite `volume-control.tsx` on the SeekBar pattern (invisible
...
- [x] 5.1 EQ/SCROB toggles become 56x56 mech buttons (icon + sublabel,
...
- [x] 6.1 Brand row (RadioReceiverIcon + "Patifon"), three-button latching
...
- [x] 7.1 Mode tabs as latched text mech chips; station card and lyrics
      colors per the canon; visualizer controls on the mech skin.
      Verify: app-modes/lyrics/radio suites green.

## 8. Vinyl deck

- [x] 8.1 Sync `vinyl-deck.tsx` to the canon: plinth/screws/platter
...
- [x] 9.1 `<title>` + PWA manifest name/short_name -> "Patifon"; update
      the e2e title assertion. Verify: pwa suite green; title shows
      "Patifon".
- [x] 9.2 Adopt canonical token names in every restyled file, delete the
