...
### Requirement: Seek affordance for library playback

For library tracks with cached peaks the seek affordance SHALL be the
waveform strip; otherwise the existing plain seek control SHALL be used.
Seek behavior (ratio mapping, buffer display) SHALL be unchanged.

#### Scenario: Same seek semantics
