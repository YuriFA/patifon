...
- [x] 1.1 Popup hook/shared component: toggle on button activation,
...
- [x] 2.1 `EqualizerPopup` island: ten native vertical band sliders
...
- [x] 3.1 `ScrobblingPopup` island over `scrobbling/settings` + `api` +
...
- [x] 4.1 Knob widget in the volume island: `role="slider"`, 0..100
...
- [x] 5.1 `RadioView` island for station rows (results, saved, pinned
...
- [x] 6.1 `PlaylistsView` island using the shared row primitives: index,
...
- [x] 7.1 `Recommendations` island (state + rows from signals): loading,
...
- [x] 8.1 Full matrix `npm run lint && npm run typecheck &&
npm run format:check && npm run build && npm run test:e2e`. Verify:
      exit 0.
- [x] 8.2 Debt sweep + visual pass: no phase-0 hex values outside `:root`
