...
test.describe("scrobbling", () => {
  ...
  test("submits playing-now on play and a completed listen at the natural end", async ({
    page,
  }) => {
    const requests = mockListenBrainz(page);
    await page.goto("/");
  ...
  test("toggle off stops every submission", async ({ page }) => {
    ...
    await expect(page.locator(".player-controls__btn_scrobbling")).not.toHaveClass(
      /scrobbling-on/u,
    );
